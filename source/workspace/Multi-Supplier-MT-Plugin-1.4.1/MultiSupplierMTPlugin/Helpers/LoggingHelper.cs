...
namespace MultiSupplierMTPlugin.Helpers
{
    static class LoggingHelper
    {
        ...
        {
            _logger.LogSpecial(message, "VERBOSE", _logger.EnableVerboseRuntimeLog);
        }

        public static void Api(string message)
        {
            _logger.LogSpecial(message, "API", _logger.EnableApiRequestResponseLog);
        }
        ...
        public static bool EnableVerboseRuntimeLog
        {
            get => _logger.EnableVerboseRuntimeLog;
            set => _logger.EnableVerboseRuntimeLog = value;
        }

        public static bool EnableApiRequestResponseLog
        {
            get => _logger.EnableApiRequestResponseLog;
            set => _logger.EnableApiRequestResponseLog = value;
        }
    ...
    sealed class Logger : IDisposable
    {
        ...
        {
            Log(message, logLevel, null, false);
        }

        public void LogSpecial(string message, string category, bool enabled)
        {
            if (!enabled) return;

            Log(message, LogLevel.Info, category, true);
        }

        private void Log(string message, LogLevel logLevel, string category, bool bypassMinLogLevel)
        {
            if (!_isInitialized || !Enable || (!bypassMinLogLevel && logLevel < MinLogLevel)) return;
            ...
            try
            {
                if (!_messageQueue.IsAddingCompleted)
                {
                    ...
                    string categoryPart = string.IsNullOrWhiteSpace(category) ? string.Empty : $" [{category}]";
                    _messageQueue.Add($"{timestamp} [{logLevel.ToString().ToUpper()}]{categoryPart} - {message}");
                }
            }
            catch { } // 吞掉异常
        }
